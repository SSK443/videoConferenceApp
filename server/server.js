...
//         const isAlreadyInRoom = roomParticipants[roomId].some(p => p.userId === userId);

//         if (isAlreadyInRoom) {
//           console.log(`User ${user.username} is already in room ${roomId}. Ignoring duplicate join event.`);
//           return; 
//         }
...
//           role: user.role,
//           userId: userId,
//           isMuted: false, // Initial mute status
//         };
...
//                 socket.to(roomId).emit('user-mute-status-changed', { peerId, muted });
//             }
//         });
...
//             const sender = roomParticipants[roomId]?.find(p => p.socketId === socket.id);
//             if(sender && sender.role === 'admin') { io.to(targetSocketId).emit('force-mute'); }
//         });
//         socket.on("admin-cam-off-user", ({ targetSocketId }) => {
//             const sender = roomParticipants[roomId]?.find(p => p.socketId === socket.id);
//             if(sender && sender.role === 'admin') { io.to(targetSocketId).emit('force-cam-off'); }
//         });
//         socket.on("admin-kick-user", ({ targetSocketId }) => {
//             const sender = roomParticipants[roomId]?.find(p => p.socketId === socket.id);
//             if(sender && sender.role === 'admin') { io.to(targetSocketId).emit('force-kick'); }
//         });
//       });
//     } catch (err) {
...
const jwt = require("jsonwebtoken");
const User = require("./models/user.model");
const Poll = require("./models/poll.model"); // <-- IMPORT THE NEW POLL MODEL
...
io.on("connection", (socket) => {
  socket.on("join-video-room", (roomId, peerId, token) => {
    ...
    try {
      ...
      User.findById(userId).then((user) => {
        ...
        socket.on('create-poll', async (pollData) => {
            const creator = roomParticipants[roomId]?.find(p => p.socketId === socket.id);
            if (!creator) return;
        ...
        socket.on('vote-poll', async ({ pollId, optionIndex }) => {
            const voter = roomParticipants[roomId]?.find(p => p.socketId === socket.id);
            if (!voter) return;

            const poll = await Poll.findById(pollId);
            if (poll && poll.isOpen) {
                // Prevent user from voting twice
                const hasVoted = poll.options.some(opt => opt.voters.includes(voter.userId));
                if (hasVoted) return;
        ...
        socket.on('end-poll', async ({ pollId }) => {
            ...
            if (poll && sender && (poll.createdBy.userId === sender.userId || sender.role === 'admin')) {
                poll.isOpen = false;
                await poll.save();
    ...
    } catch (err) { console.error("JWT verification failed for join-video-room", err); }
  });
});
